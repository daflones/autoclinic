...
-- Garantir funções auxiliares (caso não tenham sido aplicadas no Supabase)
do $$
begin
  if not exists (
    select 1
    ...
    where n.nspname = 'public'
      and p.proname = 'app_admin_profile_id'
      and p.pronargs = 0
  ) then
    execute $fn$
      create function public.app_admin_profile_id()
      returns uuid
      language sql
      stable
      security invoker
  ...
  if not exists (
    ...
    join pg_namespace n on n.oid = p.pronamespace
    where n.nspname = 'public'
      and p.proname = 'match_admin_profile'
      and p.pronargs = 1
  ) then
    execute $fn$
      create function public.match_admin_profile(target uuid)
      returns boolean
      language sql
      stable
      security invoker
